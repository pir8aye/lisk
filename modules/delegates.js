...
/**
 * Checks if block was generated by the right delegate.
 * @param {block} block - Block we are checking
 * @param {Array} delegatesList - Generated list of active delegates
 * @param {function} cb - Callback function.
 * @returns {setImmediateCallback} error message | cb
 */
__private.validateBlockSlot = function (block, delegatesList, cb) {
	var currentSlot = slots.getSlotNumber(block.timestamp);
	var delegatePublicKey = delegatesList[currentSlot % slots.delegates];

	if (delegatePublicKey && block.generatorPublicKey === delegatePublicKey) {
		return setImmediate(cb);
	} else {
		library.logger.error('Expected generator: ' + delegatePublicKey + ' Received generator: ' + block.generatorPublicKey);
...
__private.getBlockSlotData = function (slot, height, cb) {
	...
	for (; currentSlot < lastSlot; currentSlot += 1) {
		var index = currentSlot % slots.delegates;
		var delegatePublicKey = __private.delegatesList[index];

		if (delegatePublicKey && __private.keypairs[delegatePublicKey]) {
			return setImmediate(cb, null, {time: slots.getSlotTime(currentSlot), keypair: __private.keypairs[delegatePublicKey]});
		}
	}
...
/**
 * Execute query that generates delegates list of previous round.
 * @param {number} round - Round to use as seed for generating list
 * @param {function} cb - Callback function.
 * @returns {setImmediateCallback} err | generated delegate list
 */
Delegates.prototype.generateDelegateListOfPreviousRound = function (round, cb) {
	library.db.query(sql.delegateListOfPreviousRound, {round: round}).then(function (result) {
		return setImmediate(cb, null, result[0].list);
	}).catch(function (err) {
		return setImmediate(cb, err);
	});
};
...
/**
 * Generates delegates list and checks if block was generated by the right delegate,
 * decide which delegates list to use - from current or previous round.
 * @param {block} block - Block that we need to check
 * @param {block} lastBlock[=null] - Block we compare to round with (in case we deal with fork)
 * @param {function} cb - Callback function.
 * @returns {setImmediateCallback} error message | cb
 */
Delegates.prototype.validateBlockSlot = function (block, lastBlock, cb) {
	var nextBlockRound = slots.calcRound(block.height);
	var lastBlockRound = lastBlock ? slots.calcRound(lastBlock.height) : null;

	// If lastBlock is present - block is in fork state with lastBlock, so check if we need to validate
	// against previous round, first check is for fork 5, second for fork 1
	if (lastBlock && (lastBlock.height % slots.delegates === 0 || lastBlockRound < nextBlockRound)) {
		// Check if block was generated by the right active delagate from previous round
		self.generateDelegateListOfPreviousRound(lastBlockRound, function (err, delegatesList) {
			if (err) {
				return setImmediate(cb, err);
			} else {
				__private.validateBlockSlot(block, delegatesList, cb);
			}
		});
	} else {
		// Check if block was generated by the right active delagate
		__private.validateBlockSlot(block, __private.delegatesList, cb);
	}
};
